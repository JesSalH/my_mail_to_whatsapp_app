...
class EmailBroker:
    ...
    def notify_new_messages(self) -> None:
        """Called after Gmail push → notify user of count only."""
        retrieved = self.state.get_messages_by_status("retrieved")
        count = len(retrieved)
        self.notifier.notify_new_messages(count)

    def deliver_new_summaries(self) -> None:
        """On user request → summarize and deliver new messages."""
        summaries = self.summarizer.summarize_new_emails()
